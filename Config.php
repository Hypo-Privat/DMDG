...
function writeLine($dbconf)
{
     ...
     */
    // ---------------------------------------------------------------
    $fp = fopen('../locale/co-testxx.json', 'w');
    if (! $handle = fopen($fp, "w")) {
         $json_response = json_encode('error');
         print "Kann die Datei $filename nicht öffnen";
         exit();
        }
