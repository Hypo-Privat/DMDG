		...
		$.getJSON( "../locale/row_array.json", function( System ) {
			//alert("System.CREATOR - " + System.CREATOR );
					...
					$('.UpdateDetail').html('<b> ' + Sprache.resulttabreq 
						...
						/*
						 * + ' <b >Database</b><select id="database"><option
						 * selected value="o">ohne</option>' + '<option
						 * value="y">yes</option><option value="n">no</option></select>'
						 */
						
						// no update possible
						+ '<tr> <td>'+ Sprache.db + '</td> <td>' + System.CREATOR + '</td><td>'+ Sprache.ts + '</td> <td>' + System.TBSPACE + '</td>' 
						+ '<td>'+ Sprache.entity + '</td> <td>' + System.NAME + '</td></tr>' 
						+ '<tr><td>'+ Sprache.date + '</td> <td>' + System.CTIME + '</td>' + '<td>'+ Sprache.lastupd +'</td> <td>' + System.ALTER_TIME + '</td>' 
						+ '<td>' + Sprache.tabtyp + '</td> <td><input type="text" id="TTYPE" maxlength="20" value="' + daten.TTYPE + '" placeholder="' + daten.TTYPE + '"></td></tr>' 
						+ '<tr><td>'+ Sprache.runstats + '</td> <td>' + System.STATISTICS_PROFILE + '</td>' + '<td>'+ Sprache.laststats + '</td> <td>' + System.STATS_TIME + '</td>' 
						+ '<td>'+ Sprache.records + '</td> <td>' + System.COLCOUNT + '</td></tr>' 
						...
						+ '<tr><td>'+ Sprache.dmdg + '</td> <td>'
						+'<select id="TUSE_dmdg"><option selected value="' + daten.TUSE_dmdg +'">' + daten.TUSE_dmdg 
						+'</option><option value="o">ohne</option><option value="y">yes</option><option value="n">no</option></select></td>'
						...
						+ '<tr><td>'+ Sprache.remarks + '</td> <td>'  // option
																		// wäre
																		// Remarks
																		// zu
																		// holen
...
/*
 * //Ausgeben Indexes getTableIndex(database, table, env); var vers = ""; for
 * (var i = 0, len = IndexColumn.length; i < len; i++) {
 * //alert(VersionColumn[i]); var erg = VersionColumn[i]; //alert("var erg" +
 * erg); index += erg; } //alert(vers ); $todo = 'updateIndex' // NAME, TBNAME
 * ,COLNAMES , UNIQUERULE , CLUSTERFACTOR , STATS_TIME , LASTUSED
 * 
 * $('.UpdateIndex').html('<form id="updateIndex" method="get"
 * accept-charset="utf-8" onsubmit="TableIndex(' + "'" + $todo + "'" + ')">' + '<input
 * type="hidden" id="func" value="updateIndex">'
 *  + '<table border="1" width="100%"><colgroup>' + '<col
 * style="background-color: lightgray" width="10%"> ' + '<col width="10%"> ' + '
 * <col width="70%"> ' + ' <col width="10%"> ' + '<col width="10%"> ' + '</colgroup><thead
 * style="background-color: gray"><th>Name</th><th>Columns</th>' + '<th>Uniqe</th><th>Cluster</th><th>Statisics</th></thead>' + '<tbody>' +
 * index +'</tbody></table>' + '<a style="color:red" id="addVer" >add row</a>' + '<br>
 * <b><input type="submit" value="Update Change Version" ></form></b>' );
 */
...
function TableUpdate(todo) {
	// alert('TableUpdate todo = ' + todo);
		...
		if (todo === 'updateTable') {
			...
			var TUSE_dmdg = document.getElementById("TUSE_dmdg").value;
		...
		if (todo === 'updateColumn') {
			...
			// alert ("TableColumnName = " + TableColumnName[i] + "
			// COL_DESCRIPTION = " + COL_DESCRIPTION );
			// alert (i + " COLNO = " + COLNO + " TableColumnName = " +
			// TableColumnName[i] + " COL_DESCRIPTION = " + COL_DESCRIPTION );
			
			// alert( COL_NAME + " maxcolNr = " + maxcolnr + " colno = " +
			// COLNO);
			var func = 'insDocCol';
