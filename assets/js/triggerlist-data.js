...
/*verwendung von local storage macht das ganze einfacher
 ...
 window.localStorage.setItem('trigger', 'TUCC0001');
 */

/*
$('#TabReq').html('<b>Please select which Database triggers you like to see<b> <br>'
+ '<div ><form id="TabReq"  method="get" accept-charset="utf-8" onsubmit="triggerRequest()">' 
+ ' <input type="hidden" id="func" value="TabReq">' 
+ '	<b >Database</b><select id="database"><option selected value="DDESS00">ESS</option>' 
+ '<option value="DDUCC00">UCC</option><option value="DB2INST1">DB2INST1</option><option value="SYSIBM">SYSIBM</option></select>' 
+ '	<b><input type="submit" value="Send" ></b></form></div>'
);
*/

$('#TriggerReq').html(Sprache.welcome_list)
/*
...
var trigger = window.localStorage.getItem('trigger');
...
$('.TriggerListHead').html('<tr><td>'+ Sprache.trigger + '</td>'		
		+'<td>DDB2</td>' 
		+'<td>TDB2</td>'
		+'<td>QDB2</td>'
		+'<td>PDB2</td>'
...
function getEnv(database,  env) {
	...
	$.ajax({
		...
		error: function(data, req, status, err) {
			alert('Something went wrong !!! gettriggerColumn ' + data + " req = " + req + " status = " + status + " err = " + err);
 ...
 */
function calltest(database, env, trigger) {
	//alert ('hier');
	//alert('DB = ' + database + 'env = ' + env + '   trigger = ' + tab);
	window.localStorage.setItem('env', env);
	window.localStorage.setItem('database', database);
	window.localStorage.setItem('trigger', trigger);

	window.location.href = "triggerdetail.html";
}
