...
// Einlesen Dokumentation Detail
function getTableSystem(database, table, env, todo) {
	...
	$.ajax({
		...
		async: false,
		 success: 		
			 function(data) { $.each(data,
				function(counter, daten) {				
			 });  //Function each
			
		},
		error: function(data, req, status, err) {
					alert('Something went wrong !!! getTableSystem ' + data + " req = " + req + " status = " + status + " err = " + err);
					window.location.assign = "index.html";
		}
	}); //ajax

}
// Einlesen Table detail on Environment level
function getTableInfo(database, table, env, todo) {
	...
	$.ajax({
		...
		error: function(data, req, status, err) {
					alert('Something went wrong !!! getTableInfo ' + data + " req = " + req + " status = " + status + " err = " + err);
					window.location.assign = "index.html";
				}
	}); //ajax
}
				 ...
				 success : function(data) {
					...
					// TableColumnName important for
							...
							TableColumn[counter] = ('<tr> <td>'
										...
										+ '<input type="hidden" id="COLNO" value="'	+ daten.COLNO + '">'
										...
										+ '<td> <textarea id="COL_INFO'	+ daten.COLNO + '"  value="'	+ daten.COL_INFO + '" cols="50" rows="5" maxlength="3000" >'
										+ daten.COL_INFO 
										+ '</textarea> </td></tr>'
												// + '<tr><td> '+ daten.REMARKS
												// + '"></td></tr>'
